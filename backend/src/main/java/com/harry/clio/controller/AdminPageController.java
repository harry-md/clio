...
public class AdminPageController {
    ...
    @GetMapping("/")
    public String index(
            @RequestParam(defaultValue = "MONTH") RevenuePeriodType type,
        ...
        if (selectedYear == today.getYear()) {
            defaultPeriod =
                    type.isMonth() ? today.getMonthValue() : (today.getMonthValue() - 1) / 3 + 1;
        }
        ...
        model.addAttribute(
                "stats",
                statService.getPlatformRevenue(type, selectedPeriod, selectedYear, pageable));
